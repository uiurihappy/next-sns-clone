...
export default function NavMenu() {
  // url에 따라 움직이며 활성화된 링크따라 Active, ActiveLink라고 한다.
  const segment = useSelectedLayoutSegment();
  // 자식 router까지 필요하면 useSelectedLayoutSegments -> ['compose', 'tweet']
  //   console.log(segment);
  // me 같은 경우 axios나 React Query를 이용해 해당 유저에 대한 정보를 fetch하면 될 듯함
  const me = {
    // 임시로 내 정보 있는것처럼
    id: 'ybcha',
  };
      ...
      <li>
        <Link href="/home">
          <div className={style.navPill}>
            {/* home에 있을 경우 bold 처리, 그 외는 plain */}
            {segment === 'home' ? (
              <>
                <svg
                  width={26}
